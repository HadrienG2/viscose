            ...
            ($($block_size_pow2:expr),*) => {$(
                {
                    ...
                    }
                    {
                        ...
                        bench_local_floats::<BLOCK_SIZE>(c, "square", flat_name, |b: &mut Bencher, slice| {
                            flat_pool.run(|scope| b.iter(|| bench::square_pool(scope, pessimize::hide(slice))))
                        });
                    }
                    {
                        let hierarchical_pool = make_hierarchical_pool();
                        bench_local_floats::<BLOCK_SIZE>(c, "square", hierarchical_name, |b: &mut Bencher, slice| {
                            hierarchical_pool.run(|scope| b.iter(|| bench::square_pool(scope, pessimize::hide(slice))))
                        });
                    }
                }
            )*};
